      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute right-0 mt-2 w-64 max-h-64 bg-gray-800 border border-gray-700/50 rounded-lg shadow-lg z-50 overflow-y-auto custom-scrollbar"
            style={{
          ...
          >
            ...
            <div>
              {history.length === 0 ? (
                <div className="px-4 py-3 text-sm text-gray-400">Tidak ada riwayat pencarian.</div>
              ) : (
                <>
                  {history.map((item, index) => (
                    <div
                      key={index}
                      className="flex items-center justify-between px-4 py-2 text-sm text-gray-200 hover:bg-gray-700/80 hover:text-blue-300 transition-all duration-150"
                    >
                      <button
                        onClick={() => {
                          onHistoryClick(item);
                  ...
                  <div className="border-t border-gray-700/50">
                    <button
                      ...
                      className="w-full px-4 py-2 text-sm text-red-400 hover:bg-gray-700/80 transition-all duration-150"
                    >
                      Hapus Semua Riwayat
                    </button>
                  </div>
